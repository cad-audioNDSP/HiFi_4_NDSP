	...
	const float32_t  sigma2)
{
	...
	for (m = 0; m < 4; m++)
	{
		Pxtb = (xtbool *)xtbtbl - m;
		pApm = (xtfloatx2 *)(A + m);
		pApn = (xtfloatx2 *)(A);

		Acc0 = Acc1 = XT_CONST_S(0);
		_MOVT64_SX2(Acc0, sigma_w_zeros, xtbool_loadi(Pxtb, 4 - 1));
		_MOVT64_SX2(Acc0, zeros_w_sigma, xtbool_loadi(Pxtb, 4 - 0));
		_MOVT64_SX2(Acc1, sigma_w_zeros, xtbool_loadi(Pxtb, 4 + 1));
		_MOVT64_SX2(Acc1, zeros_w_sigma, xtbool_loadi(Pxtb, 4 + 2));

		for (p = 0; p < 4; p++)
		{
			_L32_SX2_IP(Apm, pApm, 4 * SZ_F32);
