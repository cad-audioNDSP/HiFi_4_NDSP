                 ...
                 int qXYR)
{
    ...
    ae_int32x2  r0reim, r1reim, xreim,t,d;
    ae_int64 A_re, A_im;
    ...
    for (k = N - 1; k >= 0; k-=2)
    {
        ...
        R-=2;
        t=AE_L32X2_I ((const ae_int32x2*)y,sizeof(ae_int32x2));        
        B0_re=AE_SLAA64S(AE_MUL32_HH(t,1),qXYR); ep0_re=AE_SEXT72(B0_re);
        B0_im=AE_SLAA64S(AE_MUL32_LL(t,1),qXYR); ep0_im=AE_SEXT72(B0_im);
        AE_L32X2_XP(t,castxcc(ae_int32x2,y),-2*(int)sizeof(ae_int32x2));        
        B1_re=AE_SLAA64S(AE_MUL32_HH(t,1),qXYR); ep1_re=AE_SEXT72(B1_re);
        B1_im=AE_SLAA64S(AE_MUL32_LL(t,1),qXYR); ep1_im=AE_SEXT72(B1_im);
        p=-(N-1)*(int)sizeof(ae_int32x2);
        for (m = N-1; m >k; m--)
        {
            AE_L32X2_XP(xreim,castxcc(ae_int32x2,x),-1*(int)sizeof(ae_int32x2));
            r0reim=AE_L32X2_I(pR,sizeof(ae_int32x2));
            AE_L32X2_XP(r1reim,pR,p);
            p+=(int)sizeof(ae_int32x2);
            t=AE_SEL32_LH(xreim,xreim);
            AE_MULSSD32EP_HH_LL(ep0_im,B0_im,t,r0reim);
            AE_MULSSD32EP_HH_LL(ep1_im,B1_im,t,r1reim);
            AE_MOVT32X2(xreim,AE_NEG32S(xreim),AE_MOVBA2(2));
            AE_MULAAD32EP_HH_LL(ep0_re,B0_re,xreim,r0reim);
            AE_MULAAD32EP_HH_LL(ep1_re,B1_re,xreim,r1reim);
        }
        ...
        xreim=d;
        AE_S32X2_XP(xreim,castxcc(ae_int32x2,x),-1*(int)sizeof(ae_int32x2));

        AE_MULSSD32EP_HH_LL(ep1_im,B1_im,AE_SEL32_LH(xreim,xreim),r1reim);
        AE_MOVT32X2(xreim,AE_NEG32S(xreim),AE_MOVBA2(2));
        AE_MULAAD32EP_HH_LL(ep1_re,B1_re,xreim,r1reim);
        d_exp=D[0].s.im;
        AE_L32_XP(d,castxcc(ae_int32,D),-2*(int)sizeof(int32_t));
        ...
        xreim=d;
        AE_S32X2_XP(xreim,castxcc(ae_int32x2,x),(N-k)*(int)sizeof(ae_int32x2));
    }
} /* cmatcholbkwsubst10x10_32x32() */
