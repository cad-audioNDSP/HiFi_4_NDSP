...
#else
{
    ...
    for (k = N - 1; k >= 0; k--)
    {
        ...
        for (n=0; n<N; n+=2)
        {
            ...
            for (m = N - k - 2; m>=0; m--)
            {
                xreim1=AE_L32X2_I (pX , sizeof(ae_int32x2));
                AE_L32X2_XP(xreim0,pX ,-N*(int)sizeof(ae_int32x2));
            ...
            AE_S32X2_I (AE_TRUNCA32X2F64S(B_re1,B_im1,d_exp+5),pX,sizeof(ae_int32x2));
            AE_S32X2_XP(AE_TRUNCA32X2F64S(B_re0,B_im0,d_exp+5),pX,0);
        }
        __Pragma("no_reorder")
    }
}
