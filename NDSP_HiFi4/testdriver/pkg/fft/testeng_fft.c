...
const void* GetFFT_handle(int N,
    int FFT_Id);
...
int te_create_fft( tTestEngContext * context )
{
  ...
  if ( res )
  {
    int tblIx;
    tVec tbl_fp64c;
    const char *dir=context->isFull ? FULL_VECTOR_DIR :BRIEF_VECTOR_DIR;
    char *fname;
    ...
    fname=(char*)malloc(strlen(dir)+64+2);
    for ( tblIx=0; tblIx<tblNum && res; tblIx++ )
    {
      ...
      sprintf(fname,"%s/%s",dir,twdFname);
      if ( !r )
      {
